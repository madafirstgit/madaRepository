...
public class cmdSet {
     ...
     * @param line (text to print)
     */
    public static void print_text(String line)
    {
    ...
    public static byte[] print_sample1()
    {
        ...
        byte[] a = init_printer();
        byte[] b = select_fontA();
        byte[] c = underline_1dot_on();
        byte[] d = justification_center();
        test = "Sample Receipt 1";
        print_line(test);
        test = "Umlaute";
        print_line(test);
        byte[] e = double_height_width_on();
        test = "ÄÖÜß";
        print_line(test);
        byte[] f = double_height_width_off();

        byte[] g = feedpapercut();
        
        byte[] b_1 = byteMerger(a, b);
        byte[] c_1 = byteMerger(b_1, c);
        byte[] d_1 = byteMerger(c_1, d);
        byte[] e_1 = byteMerger(d_1,e);
        byte[] f_1 = byteMerger(e_1,f);
        byte[] g_1 = byteMerger(f_1,g);
        return g_1;
    }


    public static void print_sample()
    {
        String test = null;
