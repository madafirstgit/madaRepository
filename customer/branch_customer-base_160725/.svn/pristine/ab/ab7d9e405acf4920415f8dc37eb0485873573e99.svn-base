/**
 * GenCreateInterceptor.java
 * cn.vko.cache.dao.mybatis
 * Copyright (c) 2015, 北京微课创景教育科技有限公司版权所有.
 */

package com.pzj.base.common.utils.mybatis;
...
import com.pzj.base.common.utils.ApplicationUtil;
import com.pzj.base.common.utils.DateUtil;
import com.pzj.base.common.utils.dbutils.PrimarykeyGenerator;
...
public class GenCreateInterceptor implements Interceptor {
					...
					if (metaObject.hasGetter(propertyName)) {
						if (!configuration.isUseGeneratedKeys()
							...
							metaObject
									.setValue(
											propertyName,
											genID(getDbName(mappedStatement,
													invocation),
													getTName(boundSql)));
						}
						...
						if (!configuration.isUseGeneratedKeys()
							...
							boundSql.setAdditionalParameter(
									propertyName,
									genID(getDbName(mappedStatement, invocation),
											getTName(boundSql)));
						}
					}
				}
	...
	public String getTName(BoundSql boundSql) {
		...
		String[] sqls = sql.split("\\(")[0].replaceAll("[\\s]+", " ").split(
				"\\s");
		return sqls[sqls.length - 1].trim();
	}

	public String getDbName(MappedStatement mappedStatement,
			Invocation invocation) {
		Executor exe = (Executor) invocation.getTarget();
		Connection connection = null;
		String url = "";
		try {
			connection = getConnection(exe.getTransaction(),
		...
		return url.substring(
				(url.lastIndexOf("/") + 1),
				(url.lastIndexOf("?") == -1 ? url.length() : url
						.lastIndexOf("?")));
	}

	private synchronized long genID(String dbName, String tName) {
		primarykeyGenerator = (PrimarykeyGenerator) ApplicationUtil
				.getBean("primarykeyGenerator");
		Long generateId = primarykeyGenerator.generateId(dbName, tName);
		log.info(String.format("===GenerateID-KEY:===[%s_%s:%s]", dbName,
				tName, generateId));
		return calculation(generateId);
	}

	private static final int yearShift = 64 - 24;

	private static final int typeShift = yearShift - 8;

	private Long calculation(Long id) {
		id &= Long.MAX_VALUE;
		id |= (Long.valueOf(properties.getProperty("flag")) << typeShift);
