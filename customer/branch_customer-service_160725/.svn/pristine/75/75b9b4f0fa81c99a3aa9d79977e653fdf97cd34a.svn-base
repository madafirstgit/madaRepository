...
import com.pzj.dao.ISysUserRelationMapper;
...
public class SysUserRelationServiceImpl extends
        BaseRelationshipServiceImpl<SysUserRelation, ISysUserRelationMapper>
    ...
    public Map<String, String> getRelationIds(List<SysUserRelation> records) {
        ...
        if (StringUtils.isNotBlank(relIds)) {
            map.put(UserGlobalParam.ChannelMapKeyParam.REF_MAP_KEY,
                    relIds.substring(0, relIds.length() - 1));
        }
        if (StringUtils.isNotBlank(types)) {
            map.put(UserGlobalParam.UserMapKeyParam.RELT_MAP_KEY,
    ...
    @Override
    public Long delAuthBatch(List<SysUserRelation> records,
    ...
    @Override
    public Long insertAuthBatch(List<SysUserRelation> records,
            boolean syncOtherRelation) {
        Long num = 0l;
        String regx = ",";
        try {
            ...
            Map<String, SysUserRelation> havMap = listTMap(findByIds(idsMap),
                    regx);
    ...
    @Override
    public PageList<SysUserRelation> queryAuthPageByParamMap(PageModel pager,
            SysUserRelation entity) {
        ...
        PageList<SysUserRelation> pagelist = new PageList<SysUserRelation>(
                listBean, pageObj);
        return pagelist;
    }
    ...
    //
    // @Override
    // public ISysUserRelationMapper getDefaulteMapper() {
    // return sysUserRelationMapper;
    // }
