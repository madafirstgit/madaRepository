...
import com.pzj.base.common.global.GlobalDict.SalesTool;
...
import com.pzj.dao.SysUserMapper;
...
public class UserServiceImpl extends BaseUserServiceImpl<SysUser, SysUserMapper> implements IUserService {
	...
	@Override
	public PageList<SysUser> findRefCustomerByRelation(SysUserRelation relation, SysUser user, boolean isFindMaster, PageModel pm) {
		if (user == null)
			return null;
	...
	public PageList<SysUser> findUserForPMS(SysUser user, SysRole role, PageModel page) {
		...
		map.put("bParam", user);
		map.put("role", role);
		setObjectDictReltionQueryParam(map, user);
		...
		try {
			...
			if (idsMap != null) {
				...
				idsMap.put(UserGlobalParam.ChannelMapKeyParam.RELATION_TYPE_KEY,
						UserGlobalParam.ChannelMapKeyParam.CHANNEL_USER_RELATION_TYPE);
				// 3.3 创建新关系删除无效的旧关系
				num += labelRelationService.updateAuthBatchBySupplierIds(idsMap, relationList,new ArrayList<Long>(supplierIds));
			}
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw e;
