...
public class AuthorityUtil {
    ...
    public static Strategy changeTStrategy(SysChannelStrategy sbean) throws Exception{
    	if(sbean == null){
    		return null;
    	}
    	
    	Strategy bean = new Strategy();
    	PropertyUtils.copyProperties(bean, sbean);
    	List<SysRebateStrategy> rebateList = sbean.getRebateStrategyList();
    	if(rebateList != null){
    		List<RebateStrategy> rebates = new ArrayList<RebateStrategy>();
    		for(SysRebateStrategy srebate : rebateList){
    			RebateStrategy rebate = new RebateStrategy();
    			PropertyUtils.copyProperties(rebate, srebate);
    			rebates.add(rebate);
    		}
    		bean.setRebateStrategyList(rebates);
