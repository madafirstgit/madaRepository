...
public class AuthorityUtil {
    ...
    public Map<String, Object> getRebatePlusIdList(
            List<RebatePlus> records) {
        if (records == null || records.isEmpty()) {
            return null;
        }
        ...
        List<String> extraRebateIdList = new ArrayList<String>();
        ...
        StringBuffer extraRebateIds = new StringBuffer();

        for (RebatePlus record : records) {
            String strategyId = record.getStrategyId();
            String productId = record.getProductId();
            String channelId = record.getChannelId();
            String extraRebateId = record.getExtraRebateId();
            ...
            if (StringUtils.isNotBlank(extraRebateId)
                    && extraRebateIds.indexOf(extraRebateId + ",") < 0) {
                extraRebateIds.append(extraRebateId).append(",");
                extraRebateIdList.add(extraRebateId);
            }
        }
        Map<String, Object> map = new HashMap<String, Object>();
        if (StringUtils.isNotBlank(strategyIds)) {
            map.put(ChannelMapKeyParam.STRATEGY_MAP_KEY, strategyIdList);
        }
        if (StringUtils.isNotBlank(productIds)) {
            map.put(ChannelMapKeyParam.PRODUCT_MAP_KEY, productIdList);
        }
        if (StringUtils.isNotBlank(channelIds)) {
            map.put(ChannelMapKeyParam.CHANNEL_MAP_KEY, channelIdList);
        }
        if (StringUtils.isNotBlank(extraRebateIds)) {
            map.put(ChannelMapKeyParam.EXTRA_REBATE_MAP_KEY, extraRebateIdList);
        }
        return map;
     ...
     * 根据渠道id获取加点返利产品关联的政策
     */
    public List<SysSpcreRelationKey> getStrategySpcreByChannel(
            List<ChannelVo> voList) throws Exception {
        List<SysSpcreRelationKey> relationList = null;
        if (voList == null || voList.isEmpty()) {
            return relationList;
        }
        String channelIds = channelVoUtil.getIds(voList);

        Map searchMap = new HashMap();
        searchMap.put(ChannelMapKeyParam.CHANNEL_MAP_KEY, channelIds);
        relationList = ispcreRelationService.findByIds(searchMap);
        return relationList;
    }
     ...
     * 根据产品id获取加点返利产品关联的政策
     */
    public List<SysSpcreRelationKey> getStrategySpcreByProductId(
            List<Long> voList) throws Exception {
        List<SysSpcreRelationKey> relationList = null;
        if (voList == null || voList.isEmpty()) {
            return relationList;
        }
        ...
        Map searchMap = new HashMap();
        searchMap.put(ChannelMapKeyParam.PRODUCT_MAP_KEY, voList);
        relationList = ispcreRelationService.findByIdList(searchMap);
        return relationList;
    }
     ...
     * 根据渠道id获取加点返利产品关联的政策
     */
    public List<SysSpcreRelationKey> getStrategySpcreByRebatePlus(
            List<RebatePlus> voList) throws Exception {
        List<SysSpcreRelationKey> relationList = null;
        if (voList == null || voList.isEmpty()) {
            return relationList;
        }
        Map<String, Object> idList = getRebatePlusIdList(voList);

        relationList = ispcreRelationService.findByIdList(idList);
        return relationList;
    }
