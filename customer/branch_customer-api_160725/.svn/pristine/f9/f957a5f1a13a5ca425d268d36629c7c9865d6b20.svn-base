...
import static com.pzj.util.ServiceUtil.checkLength;
...
public class Customer extends CommonEntity implements Serializable {
     ...
     * @throws Exception
     */
    public static SysUser createNewSysUser(Customer customer,
                                           boolean isNeedEncrypt) throws Exception {
    ...
    public static void validDataAll(Customer customer) throws ServiceException {
        ...
        checkLength(customer.getLoginName(), 64, "Customer.LoginName超过最大长度，最大为64");
        checkLength(customer.getLoginPasswd(), 64, "Customer.LoginPasswd超过最大长度，最大为64");
        checkLength(customer.getSysCode(), 64, "Customer.SysCode超过最大长度，最大为64");
        checkLength(customer.getUserType(), 64, "Customer.UserType超过最大长度，最大为64");
        checkLength(customer.getOperChargerMobile(), 32, "Customer.OperChargerMobile超过最大长度，最大为32");
        checkLength(customer.getOperChargerPhone(), 32, "Customer.OperChargerPhone超过最大长度，最大为32");
        checkLength(customer.getOperChargerFax(), 32, "Customer.ChargerFax超过最大长度，最大为32");
        checkLength(customer.getOperChargerEmail(), 32, "Customer.ChargerFax超过最大长度，最大为32");
        checkLength(customer.getLastLoginIp(), 16, "Customer.ChargerFax超过最大长度，最大为32");
        checkLength(customer.getOfficeId(), 10, "Customer.OfficeId超过最大长度，最大为32");
        checkLength(customer.getWxOpenid(), 32, "Customer.WxOpenid超过最大长度，最大为32");
        checkLength(customer.getName(), 50, "Customer.Name超过最大长度，最大为32");
        checkLength(customer.getResellerType(), 16, "Customer.ResellerType超过最大长度，最大为32");
        checkLength(customer.getProvince(), 64, "Customer.Province超过最大长度，最大为32");
        checkLength(customer.getCity(), 64, "Customer.City超过最大长度，最大为32");
        checkLength(customer.getCounty(), 64, "Customer.County超过最大长度，最大为32");
        checkLength(customer.getCorporater(), 64, "Customer.Corporater超过最大长度，最大为32");
        checkLength(customer.getCorporaterCredentials(), 16,
            "Customer.CorporaterCredentials超过最大长度，最大为32");
        checkLength(customer.getCorporaterMobile(), 16, "Customer.CorporaterMobile超过最大长度，最大为32");
        checkLength(customer.getCorporaterPhone(), 16, "Customer.CorporaterPhone超过最大长度，最大为32");
        checkLength(customer.getCorporaterEmail(), 32, "Customer.CorporaterEmail超过最大长度，最大为32");
        checkLength(customer.getBusinessCertificate(), 32,
            "Customer.BusinessCertificate超过最大长度，最大为32");
        checkLength(customer.getBusinessLicense(), 32, "Customer.BusinessLicense超过最大长度，最大为32");
        checkLength(customer.getOrgCodeCertificate(), 32,
            "Customer.OrgCodeCertificate超过最大长度，最大为32");
        checkLength(customer.getTaxCertificate(), 32, "Customer.TaxCertificate超过最大长度，最大为32");
        checkLength(customer.getOtherFiles(), 32, "Customer.OtherFiles超过最大长度，最大为32");
        checkLength(customer.getAddress(), 32, "Customer.Address超过最大长度，最大为32");
        checkLength(customer.getApproveResult(), 64, "Customer.ApproveResult超过最大长度，最大为64");
        checkLength(customer.getContractNum(), 32, "Customer.ContractNum超过最大长度，最大为32");
        checkLength(customer.getDescription(), 1024, "Customer.Description超过最大长度，最大为1024");
        checkLength(customer.getAboutUs(), 1024, "Customer.AboutUs超过最大长度，最大为1024");
        checkLength(customer.getContactWay(), 1024, "Customer.ContactWay超过最大长度，最大为1024");
        checkLength(customer.getResellerPhoneticShorthand(), 128,
            "Customer.ResellerPhoneticShorthand超过最大长度，最大为128");
        checkLength(customer.getSupplierLevel(), 100, "Customer.SupplierLevel超过最大长度，最大为100");
        checkLength(customer.getOtherFile(), 256, "Customer.OtherFile超过最大长度，最大为256");
        checkLength(customer.getOtherFile2(), 256, "Customer.OtherFile2超过最大长度，最大为256");
        checkLength(customer.getSupplierAddress(), 256, "Customer.SupplierAddress超过最大长度，最大为256");
        checkLength(customer.getSupplierDescription(), 1024,
            "Customer.SupplierDescription超过最大长度，最大为1024");
        checkLength(customer.getSupplierPy(), 128, "Customer.SupplierPy超过最大长度，最大为128");
        checkLength(customer.getSupplierNormal(), 64, "Customer.SupplierNorma超过最大长度，最大为64");
        checkLength(customer.getContractRemarks(), 1024, "Customer.ContractRemarks超过最大长度，最大为1024");
        checkLength(customer.getContractNotes(), 1024, "Customer.ContractNotes超过最大长度，最大为1024");
        checkLength(customer.getVerificationCodes(), 16, "Customer.VerificationCodes超过最大长度，最大为16");
        checkLength(customer.getUserSource(), 45, "Customer.UserSource超过最大长度，最大为45");
        checkLength(customer.getCreateBy(), 45, "Customer.CreateBy超过最大长度，最大为45");
        checkLength(customer.getUpdateBy(), 45, "Customer.OrgCodeCertificate超过最大长度，最大为45");
    }
