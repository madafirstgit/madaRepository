...
import static com.pzj.scenic.entity.ScenicBuilder.AScenicBuilder;
...
import com.pzj.customer.entity.pms.PMSCustomerQueryBuilder;
...
import com.pzj.scenic.entity.Scenic;
...
public class CustomerServiceImpl implements CustomerService {
	...
	private void setCustomerToken(Customer customer) throws Exception {
		...
		String token = customerUtil.genTokenKey(customer);// 生成token
		...
		// 通过token存入用户信息到缓存
		userRedisService.put(UserMapKeyParam.USER_KEY + token, jsonStirng,
				0L);
	...
	private PageList<Customer> convertToCustomerPageList(
			PageList<SysUser> userList) throws Exception {
		if (null != userList) {
			PageList<Customer> result = new PageList<>();
			...
			if (null != resultList) {
				List<Customer> customerList = Customer.sList2CList(resultList);
