...
public class Role extends CommonEntity implements Serializable {
    ...
    public static void validDataAll(Role role) throws ServiceException {
        validData(role);
        checkLength(role.getName(), 100, "Role.Name超过最大长度，最大为100");
        checkLength(role.getDataScope(), 1, "Role.DataScope超过最大长度，最大为1");
        checkLength(role.getCreateBy(), 64, "Role.CreateBy超过最大长度，最大为64");
        checkLength(role.getUpdateBy(), 64, "Role.UpdateBy超过最大长度，最大为64");
        checkLength(role.getRemarks(), 255, "Role.Remarks超过最大长度，最大为255");
        checkLength(role.getDelFlag(), 1, "Role.DelFlag超过最大长度，最大为1");
    }
