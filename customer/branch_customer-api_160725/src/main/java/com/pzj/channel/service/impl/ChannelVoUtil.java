...
public class ChannelVoUtil {
     ...
     * @throws Exception
     */
    public void setChannelStrategyList(ChannelVo vo) throws Exception {
        if (vo == null) {
            return;
        }
        if (vo.getId() == null) {
        ...
        PageList<SysChannelStrategy> pageList = ichannelStrategyService
                .queryPageByRefId(null, null, vo.getId(),
                        ChannelMapKeyParam.STRATEGY_CHANNEL_RELATION_TYPE);
        if (pageList == null || pageList.getResultList() == null
                || pageList.getResultList().isEmpty()) {
            return;
        }
        List<StrategyVo> list = StrategyVo
                .sList2CList(pageList.getResultList());
        vo.setSlist(list);
    }
