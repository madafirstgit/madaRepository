...
public class RollbackOccupyStockValidator implements ObjectConverter<OccupyStockRequestModel, ServiceContext, ParamModel> {
    ...
    public ParamModel convert(OccupyStockRequestModel stockModel, ServiceContext context) {
        ...
        if (CommonUtils.checkObjectIsNull(stockModel)) {
            paramModel.setErrorModel("释放库存传入OccupyStockRequestModel是空!");
            return paramModel;
        }
        if (CommonUtils.checkStringIsNullStrict(stockModel.getTransactionId())) {
            paramModel.setErrorModel("释放库存传入transactionId交易ID是空!");
            return paramModel;
        }
        if (CommonUtils.checkLongIsNull(stockModel.getProductId(), true)) {
            paramModel.setErrorModel("释放库存传入productId产品ID是空!");
            return paramModel;
        }
        if (CommonUtils.checkLongIsNull(stockModel.getStockId(), true)) {
            paramModel.setErrorModel("释放库存传入stockId库存ID是空!");
            return paramModel;
        }
        return paramModel;
    }
