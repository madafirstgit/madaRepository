...
import com.pzj.core.stock.exception.rule.GainProductStockRuleRelException;
...
public class StockBottomServiceImpl implements StockBottomService {
    ...
    public Result<Integer> syncSkuAndRuleRel(ServiceContext context) {
        ...
        //2.调用产品服务获取关系数据
        Result<ArrayList<SkuStockRelationResult>> result;
        try {
            result = skuStockService.getAllValidSkuRondaRelation();
        } catch (Throwable t) {
        ...
        if (result.isOk()) {
            ArrayList<SkuStockRelationResult> data = result.getData();
            if (null == data || data.size() == 0) {
                if (logger.isDebugEnabled()) {
                    logger.debug("relation is null.");
                }
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("sync sku and rule relation success ,response:{},context:{}.", size, context);
                }

                return new Result<Integer>(size);

            } catch (Throwable t) {

                logger.error("synchronize sku stock rule relation failed ,context:{}.", context, t);
            ...
            logger.error("call sku service error,message:{}.", result.getErrorMsg());
    ...
    public Result<Integer> autoCreateDailyStock(ServiceContext context) {
        ...
        try {
            ...
            for (int i = 0; i < size; i += 200) {
                ...
                if (!Check.NuNCollections(addStockModelList)) {
                    ...
                    while (itera.hasNext()) {
                        ...
                        //过滤数据库中已经存在的库存
                        ArrayList<Long> ruleIdsDB = stockQureyInIdsAndStockTimeEngine.queryRuleIdsByDate(ruleIds, stockTime, context);
            ...
            logger.error("auto create stock failed ,context:{}.", context, t);
