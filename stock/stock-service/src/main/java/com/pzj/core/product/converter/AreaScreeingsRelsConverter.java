...
public class AreaScreeingsRelsConverter implements ObjectConverter<ArrayList<AreaScreeingsRel>, ServiceContext, ArrayList<AreaScreeingsRelModel>> {
    ...
    public ArrayList<AreaScreeingsRelModel> convert(ArrayList<AreaScreeingsRel> list, ServiceContext context) {
        ...
        for (AreaScreeingsRel rel : list) {
            ...
            try {
                PropertyUtils.copyProperties(model, rel);
            } catch (Exception e) {
                logger.error(" copy AreaScreeingsRel properties to AreaScreeingsRelModel error , context:{}", context, e);
