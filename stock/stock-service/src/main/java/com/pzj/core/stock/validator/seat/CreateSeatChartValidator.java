...
public class CreateSeatChartValidator implements ObjectConverter<SeatChartModel, Void, Boolean> {

    /** 
     * @see com.pzj.framework.converter.ObjectConverter#convert(java.lang.Object, java.lang.Object)
     */
    @Override
    public Boolean convert(SeatChartModel seatChartModel, Void v) {
        ...
        for (String row : rowArr) {
            ...
            for (int i = 0; i < lineArr.length; i++) {
                if (i != 0) {
                    if (!"_".equals(lineArr[i]) && !lineArr[i].matches("^\\d+$")) {
                        return Boolean.FALSE;
                    }
                }
            }
