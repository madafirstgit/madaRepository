...
public class OccupySeatEngine {
    ...
    private static Logger           logger = LoggerFactory.getLogger(OccupySeatEngine.class);
    ...
    public void occupySeat(ShowModel showModel) {
        if (logger.isDebugEnabled()) {
            logger.debug("------occupySeat-- showModel:{}-----", showModel);
        }
        //检查该业务唯一id是否已经占座
        ...
        //检查库存对应的座位是否已经被占
        List<StockSeatRel> stockSeatRelList = stockSeatRelReadMapper.queryByStockAndSeats(showModel.getStockId(), showModel.getSeats());
