...
public class ActingProductCreateEngine {
    ...
    private List<AreaScreeingsRel> generaterRel(ActingProductModel model, ServiceContext context) {
        ...
        if (null == areas || areas.size() == 0 || null == screeingses || screeingses.size() == 0) {
            logger.error(" generate area screeings rel fail ,request:{},context:{}. ", model, context);
            return null;
        }
        ...
        for (AreaModel area : areas) {
            ...
            for (ScreeingsModel screeings : screeingses) {
                Long screeingsId = screeings.getId();
                if (screeingsId == null || screeingsId < 1) {
                    continue;
                }
                try {
                    ...
                    clone.setScreeingsId(screeingsId);
