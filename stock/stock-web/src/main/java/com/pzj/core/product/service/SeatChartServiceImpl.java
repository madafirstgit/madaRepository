...
public class SeatChartServiceImpl {
    ...
    public Boolean addSeatChart(SeatChartVo seatChartVo) {
        ...
        Result<Boolean> result = seatChartService.addSeatChartInitSeat(seatChartModel, ServiceContext.getServiceContext());
        if (result.isOk()) {
            flag = result.getData();
        }
        return flag;
    ...
    public List<AreaVo> queryAreaVoByScenic(String scenicName) {
        ...
        Result<ArrayList<AreaModel>> result = areaQueryService.queryAreasByWebSeatChartParam(model, ServiceContext.getServiceContext());
        if (result.isOk()) {
